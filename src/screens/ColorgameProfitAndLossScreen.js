...
import { fetchProfitLossByMarketCg } from '../services/authService.js'; // Updated import

const ColorgameProfitAndLossScreen = () => {
  ...
  const handleGetStatement = async () => {
    try {
      ...
      const response = await fetchProfitLossByMarketCg({
        gameId,
  ...
  const handleMarketNamePress = (marketId, marketName) => {
    navigation.navigate('MarketDetailsScreenCg', { 
      marketId,
      marketName 
    });
  };
    ...
    <View style={styles.mainContainer}>
      ...
      >
        <View style={styles.header}>
          <Text style={styles.headerText}>{gameName ? `${gameName.toUpperCase()} PROFIT & LOSS` : 'PROFIT & LOSS STATEMENT'}</Text>
        </View>
            ...
            {tableData.length > 0 ? (
              <>
                {tableData.map((row, index) => (
                  ...
                  >
                    <Text style={[styles.cell, styles.sportCell]}>{row.gameName || gameName}</Text>
                    <TouchableOpacity 
                    ...
                    {renderProfitLossValue(row.totalProfitLoss)}
                    {renderProfitLossValue(row.totalProfitLoss)}
                  </View>
                ))}
              </>
...
export default ColorgameProfitAndLossScreen;